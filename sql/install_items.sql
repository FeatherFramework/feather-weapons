...
SELECT 'revolver_cattleman', 'Cattleman Revolver', 'A standard single-action revolver.', 20, 1, 2, 1, 3, 'item_weapon', 'unique'
WHERE NOT EXISTS (SELECT 1 FROM `items` WHERE `name` = 'revolver_cattleman');

INSERT INTO `items`
    (`name`, `display_name`, `description`, `max_quantity`, `max_stack_size`, `weight`, `usable`, `category_id`, `type`, `instance_mode`)
SELECT 'revolver_schofield', 'Schofield Revolver', 'A sturdy top-break revolver.', 20, 1, 2, 1, 3, 'item_weapon', 'unique'
WHERE NOT EXISTS (SELECT 1 FROM `items` WHERE `name` = 'revolver_schofield');
